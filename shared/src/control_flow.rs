...
/// Elif branch: (condition, body commands)
pub type ElifBranch = (String, Vec<String>);
...
mod tests {
    ...
    mod coverage_tests {
        ...
        #[test]
        fn test_parse_if_not_starting_with_if() {
            let lines = vec!["while true; do", "echo hello", "fi"];
            let result = parse_if_statement(&lines);
            assert!(result.is_err());
            assert!(result.unwrap_err().contains("Expected 'if'"));
        }

        #[test]
        fn test_parse_while_empty_lines() {
            let lines: Vec<&str> = vec![];
            let result = parse_while_statement(&lines);
            assert!(result.is_err());
            assert!(result.unwrap_err().contains("Empty"));
        ...
        #[test]
        fn test_parse_while_not_starting_with_while() {
            let lines = vec!["if true; then", "echo hello", "done"];
            let result = parse_while_statement(&lines);
            assert!(result.is_err());
            assert!(result.unwrap_err().contains("Expected 'while'"));
        }

        #[test]
        fn test_parse_until_empty_lines() {
            let lines: Vec<&str> = vec![];
            let result = parse_until_statement(&lines);
            assert!(result.is_err());
            assert!(result.unwrap_err().contains("Empty"));
        ...
        #[test]
        fn test_parse_until_not_starting_with_until() {
            let lines = vec!["while true; do", "echo hello", "done"];
            let result = parse_until_statement(&lines);
            assert!(result.is_err());
            assert!(result.unwrap_err().contains("Expected 'until'"));
        }

        #[test]
        fn test_parse_until_missing_done() {
            let lines = vec!["until false; do", "echo hello"];
            let result = parse_until_statement(&lines);
            assert!(result.is_err());
            assert!(result.unwrap_err().contains("Missing 'done'"));
        }
        ...
        #[test]
        fn test_parse_for_not_starting_with_for() {
            let lines = vec!["while true; do", "echo hello", "done"];
            let result = parse_for_statement(&lines);
            assert!(result.is_err());
            assert!(result.unwrap_err().contains("Expected 'for'"));
        }

        #[test]
        fn test_parse_for_no_variable() {
            let lines = vec!["for ; do", "echo hello", "done"];
            let result = parse_for_statement(&lines);
            assert!(result.is_err());
        }
        ...
        #[test]
        fn test_until_with_multiline_body() {
            let lines = vec!["until false; do", "echo a", "echo b", "done"];
            let result = parse_until_statement(&lines);
            assert!(result.is_ok());
        ...
        #[test]
        fn test_if_with_empty_lines_in_body() {
            let lines = vec!["if true; then", "", "echo hello", "", "fi"];
            let result = parse_if_statement(&lines);
            assert!(result.is_ok());

            if let ControlFlow::If { then_body, .. } = result.unwrap() {
        ...
        #[test]
        fn test_while_with_empty_lines_in_body() {
            let lines = vec!["while true; do", "", "echo loop", "", "done"];
            let result = parse_while_statement(&lines);
            assert!(result.is_ok());

            if let ControlFlow::While { body, .. } = result.unwrap() {
                // Empty lines should be skipped
                assert_eq!(body, vec!["echo loop"]);
            } else {
                panic!("Expected While variant");
        ...
        #[test]
        fn test_for_with_empty_lines_in_body() {
            let lines = vec!["for i in 1 2; do", "", "echo $i", "", "done"];
            let result = parse_for_statement(&lines);
            assert!(result.is_ok());

            if let ControlFlow::For { body, .. } = result.unwrap() {
                assert_eq!(body, vec!["echo $i"]);
            } else {
                panic!("Expected For variant");
            }
        ...
        #[test]
        fn test_control_flow_while_serialization() {
            let while_stmt = ControlFlow::While {
                condition: "true".to_string(),
                body: vec!["echo loop".to_string()],
            };

            let json = serde_json::to_string(&while_stmt).unwrap();
            let restored: ControlFlow = serde_json::from_str(&json).unwrap();
            assert_eq!(while_stmt, restored);
        }

        #[test]
        fn test_control_flow_until_serialization() {
            let until_stmt = ControlFlow::Until {
                condition: "false".to_string(),
                body: vec!["echo loop".to_string()],
            };

            let json = serde_json::to_string(&until_stmt).unwrap();
            let restored: ControlFlow = serde_json::from_str(&json).unwrap();
            assert_eq!(until_stmt, restored);
        }

        #[test]
        fn test_control_flow_cfor_serialization() {
            let cfor_stmt = ControlFlow::CFor {
                init: "i=0".to_string(),
            ...
            let json = serde_json::to_string(&cfor_stmt).unwrap();
            let restored: ControlFlow = serde_json::from_str(&json).unwrap();
            assert_eq!(cfor_stmt, restored);
        }
        ...
        fn test_control_flow_debug() {
            ...
            let debug_str = format!("{:?}", while_stmt);
            assert!(debug_str.contains("While"));
        }

        #[test]
        fn test_cfor_with_spaces_in_parts() {
            let lines = vec!["for (( i = 0; i < 10; i++ )); do", "echo $i", "done"];
            let result = parse_for_statement(&lines);
            assert!(result.is_ok());
        }
