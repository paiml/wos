...
pub(crate) fn handle_parameter_expansion(
    ...
    if ch == '$' && chars.peek() == Some(&'{') && !in_single_quote {
        ...
        current_token.push(chars.next().expect("peek confirmed '{' exists")); // consume '{'
        *brace_depth += 1;
        true
    } else {
        false
...
pub(crate) fn handle_command_or_arithmetic_expansion(
    ...
    if ch == '$' && chars.peek() == Some(&'(') && !in_single_quote {
        current_token.push(ch);
        current_token.push(chars.next().expect("peek confirmed '(' exists")); // consume first '('

        // Check if this is $(( (arithmetic) or just $( (command substitution)
        if chars.peek() == Some(&'(') {
            current_token.push(chars.next().expect("peek confirmed second '(' exists")); // consume second '('
