...
/// Extended attributes map: inode -> (attribute name -> value)
type ExtendedAttributesMap = im::HashMap<InodeNumber, im::HashMap<String, Vec<u8>>>;

/// Result of resolving a parent path: (parent inode, filename)
type ResolveParentResult = Result<(InodeNumber, String), VfsError>;

/// Result of finding mount point: (VFS reference, remaining path)
type MountPointResult<'a> = Result<(&'a mut VirtualFileSystem, PathBuf), VfsError>;
...
impl VirtualFileSystem {
    ...
    pub fn new() -> Self {
        ...
        let _ = vfs.create_directory(PathBuf::from("/models"));
    ...
    fn resolve_parent(&self, path: &Path) -> ResolveParentResult {
        // Normalize path to handle ., .., and multiple slashes
        let normalized_path = Self::normalize_path(path);
        ...
        let name = components
            .last()
            .expect("components is non-empty after check above")
            .to_string();
    ...
    fn resolve_mount_mut(&mut self, path: &Path) -> MountPointResult<'_> {
        let normalized_path = Self::normalize_path(path);
