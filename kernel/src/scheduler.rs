...
/// Result type for schedule function: (updated state, optional next process)
pub type ScheduleResult = Result<(KernelState, Option<ProcessId>), KernelError>;
...
pub fn schedule(mut state: KernelState) -> ScheduleResult {
    ...
    for pid in processes_to_update {
        if let Some(mut process) = state.processes.get(&pid).cloned() {
            ...
            while let Some(signal) = process.pending_signals.next_signal() {
                ...
                // Check for custom handler (use default action if no handler registered)
                let action = match process.signal_handlers.get(&signal.number()) {
                    Some(&handler_action) => handler_action,
                    None => signal.default_action(),
                };
