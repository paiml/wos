...
impl SharedMemoryManager {
    ...
    pub fn map_segment(
        ...
        let mut mappings = self.process_mappings.get(&pid).cloned().unwrap_or_default();
    ...
    pub fn write(
        ...
        // Write the data (segment must exist since we checked mapped_processes above)
        let segment = self
            .segments
            .get_mut(&shm_id)
            .expect("segment must exist since mapped_processes check passed");
    ...
    pub fn get_process_mappings(&self, pid: ProcessId) -> Vector<ProcessMapping> {
        self.process_mappings.get(&pid).cloned().unwrap_or_default()
    }
