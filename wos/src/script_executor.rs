...
mod proptests {
    ...
    // Property test: For loop handles non-empty list sizes (echo only)
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_for_loop_echo_items(
            items in prop::collection::vec("[a-z]+", 1..20)
        ) {
            ...
            // For loops should never panic with valid lists
            prop_assert!(result.is_ok());
        }
    }

    // Property test: Echo commands with multiple arguments
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_echo_multiple_args(
            ...
            prop_assert!(result.is_ok());
            if let Ok(exec_result) = result {
                prop_assert!(!exec_result.output.is_empty());
            }
        }
    }

    // Property test: Multiple echo commands in sequence
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_multiple_echo_commands(
            lines in prop::collection::vec("[a-z]{3,10}", 1..15)
        ) {
            let commands: Vec<String> = lines.iter().map(|line| format!("echo {}", line)).collect();
            let content = format!("#!/bin/bash\n{}", commands.join("\n"));
            let script = Script {
                path: "/test.sh".to_string(),
                content,
            ...
            // Multiple commands should execute successfully
            prop_assert!(result.is_ok());
            if let Ok(exec_result) = result {
                // Output should contain newlines for multiple commands
                let output_lines = exec_result.output.lines().count();
                prop_assert!(output_lines >= 1);
            }
        }
    }

    // Property test: Scripts with comments and empty lines
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_comments_and_empty_lines(
            command in "[a-z]+",
            num_comments in 0..5usize,
            num_empty_lines in 0..5usize
        ) {
            let mut lines = vec![];
            for _ in 0..num_comments {
                lines.push("# This is a comment".to_string());
            }
            for _ in 0..num_empty_lines {
    ...
    // Property test: Variable assignment persistence in shell context
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_variable_assignment_persistence(
            var1 in "[A-Z]{3,8}",
            val1 in "[a-z0-9]{3,10}",
            var2 in "[A-Z]{3,8}",
            val2 in "[a-z0-9]{3,10}"
            ...
            let mut vars = HashMap::new();
            let mut test_executor = create_test_executor();
            let result = ScriptExecutor::execute_in_shell_context(&script, &mut vfs, &mut vars, &mut test_executor);

            // Variable assignments should persist
            prop_assert!(result.is_ok());
            prop_assert_eq!(vars.get(&var1), Some(&val1));
            prop_assert_eq!(vars.get(&var2), Some(&val2));
        }
    }

    // Property test: Multiple variable assignments with echo
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_mixed_assignments_and_commands(
            assignments in prop::collection::vec(("[A-Z]+", "[a-z]+"), 2..8)
        ) {
            let mut lines = vec![];
            for (var, val) in &assignments {
                lines.push(format!("{}={}", var, val));
                lines.push(format!("echo {}", var));
            }
            let content = format!("#!/bin/bash\n{}", lines.join("\n"));
            let script = Script {
                path: "/test.sh".to_string(),
            ...
            prop_assert!(result.is_ok());
            for (var, val) in &assignments {
                prop_assert_eq!(vars.get(var), Some(val));
            }
        }
    }

    // Property test: Script exit codes propagate correctly
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_exit_code_propagation(
            success_commands in prop::collection::vec("[a-z]+", 1..5),
            should_fail in prop::bool::ANY
        ) {
            let mut lines: Vec<String> = success_commands.iter()
                .map(|cmd| format!("echo {}", cmd))
                .collect();

            if should_fail {
                lines.push("invalid_command".to_string());
            }
            ...
            // Exit codes should propagate
            prop_assert!(result.is_ok());
            if let Ok(exec_result) = result {
                if should_fail {
                    prop_assert_eq!(exec_result.exit_code, 127);
                } else {
                    prop_assert_eq!(exec_result.exit_code, 0);
                }
            }
        }
    }

    // Property test: Variable overwriting
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_variable_overwriting(
            var_name in "[A-Z]{3,8}",
            values in prop::collection::vec("[a-z0-9]+", 2..6)
        ) {
            let mut lines: Vec<String> = values.iter()
            ...
            prop_assert!(result.is_ok());
            prop_assert_eq!(vars.get(&var_name), values.last());
        }
    }

    // Property test: Empty variable values
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_empty_variable_values(
            var_names in prop::collection::vec("[A-Z]{2,6}", 1..5)
        ) {
            let mut lines: Vec<String> = var_names.iter()
                .map(|var| format!("{}=", var))
                .collect();
            lines.push("echo done".to_string());
            ...
            for var in &var_names {
                prop_assert_eq!(vars.get(var), Some(&String::new()));
            }
        }
    }

    // Property test: Script with only variable assignments (no echo)
    proptest! {
        #![proptest_config(ProptestConfig::with_cases(100))]
        #[test]
        fn proptest_only_variable_assignments(
            assignments in prop::collection::vec(("[A-Z]+", "[a-z0-9]+"), 1..10)
        ) {
            let lines: Vec<String> = assignments.iter()
            ...
            // Script with only assignments should succeed
            prop_assert!(result.is_ok());
            if let Ok(exec_result) = result {
                prop_assert_eq!(exec_result.output, "");
                prop_assert_eq!(exec_result.exit_code, 0);
            }
            // All variables should be set with last assigned value (overwrites earlier values)
            let mut expected_vars: HashMap<String, String> = HashMap::new();
            for (var, val) in &assignments {
                expected_vars.insert(var.clone(), val.clone());
            }
            prop_assert_eq!(vars, expected_vars);
        }
    }
}
