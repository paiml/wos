...
/// Result type for sed address parsing: (address, remaining command string)
type SedAddressParseResult<'a> = Result<(SedAddress, &'a str), String>;
...
impl Sed {
    ...
    fn parse_address_and_command<'a>(&self, line: &'a str) -> SedAddressParseResult<'a> {
                ...
                if let Some(comma_pos) = line.find(',') {
                    ...
                    while i < rest.len()
                        && rest
                            .chars()
                            .nth(i)
                            .expect("index i is within rest.len()")
                            .is_ascii_digit()
                    {
                        i += 1;
                    }
                    ...
                    while i < line.len()
                        && line
                            .chars()
                            .nth(i)
                            .expect("index i is within line.len()")
                            .is_ascii_digit()
                    {
                        i += 1;
                    }
    ...
    fn parse_substitute(&self, cmd_str: &str) -> Result<SedCommand, String> {
        ...
        let delimiter = rest
            .chars()
            .next()
            .expect("rest is non-empty after check above");
