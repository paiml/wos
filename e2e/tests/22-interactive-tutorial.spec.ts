...
test.describe('WOS-304: Interactive Tutorial', () => {
  ...
  test.describe('Step 1: Welcome', () => {
    ...
    test('should close tutorial when Skip clicked', async ({ page }) => {
      ...
      const tutorialOverlay = await page.locator('#tutorial-overlay');
      await expect(tutorialOverlay).toBeHidden({ timeout: 5000 });
    });
  ...
  test.describe('Step 3: Visual Monitor', () => {
    test('should highlight process view panel', async ({ page }) => {
      ...
      await page.locator('#btn-begin-tutorial').click();
      await expect(page.locator('.tutorial-step[data-step="terminal"]')).toBeVisible({ timeout: 5000 });
      await page.locator('.tutorial-step:not(.hidden) .btn-tutorial-next').click(); // Skip terminal step
      await expect(page.locator('.tutorial-step[data-step="monitor"]')).toBeVisible({ timeout: 5000 });
    ...
    test('should prompt user to type ps command', async ({ page }) => {
      ...
      await page.locator('#btn-begin-tutorial').click();
      await expect(page.locator('.tutorial-step[data-step="terminal"]')).toBeVisible({ timeout: 5000 });
      await page.locator('.tutorial-step:not(.hidden) .btn-tutorial-next').click();
      await expect(page.locator('.tutorial-step[data-step="monitor"]')).toBeVisible({ timeout: 5000 });
  ...
  test.describe('Navigation', () => {
    ...
    test('should allow going back to previous step', async ({ page }) => {
      ...
      const backButton = await page.locator('.tutorial-step:not(.hidden) .btn-tutorial-back');
  ...
  test.describe('Retake Tutorial', () => {
    test('should have retake button in help menu', async ({ page }) => {
      ...
      await page.waitForSelector('#btn-retake-tutorial-menu', { timeout: 2000 });
    ...
    test('should restart tutorial from beginning when retake clicked', async ({ page }) => {
      ...
      await expect(page.locator('.tutorial-step[data-step="welcome"]')).toBeVisible({ timeout: 5000 });
  ...
  test.describe('Accessibility', () => {
    test('should support keyboard navigation with Tab', async ({ page }) => {
      ...
      await page.waitForFunction(() => {
        const btn = document.querySelector('#btn-begin-tutorial');
