# ============================================================================
# WOS - WASM Operating System Build System
# ============================================================================
# Makefile for building, testing, and deploying WOS
# See `make help` for available targets
# ============================================================================

# Delete partially-built targets on error
.DELETE_ON_ERROR:
...
# Get target directory from cargo config or use default
CARGO_TARGET_DIR := $(shell cat .cargo/config.toml 2>/dev/null | grep 'target-dir' | cut -d'"' -f2 || echo "target")
WASM_TARGET := $(CARGO_TARGET_DIR)/wasm32-unknown-unknown/release/wos.wasm
...
help:
	...
	@echo "  make test-proptest    Run property-based tests (10K inputs)"
	@echo "  make test-jidoka      Run Jidoka invariant tests"
	@echo "  make test-apr         Run APR runtime tests"
	@echo "  make test-vmm         Run VMM layer tests"
	@echo "  make coverage-fast    Fast coverage with llvm-cov"
	@echo "  make coverage-proptest Property test coverage"
	@echo ""
	@echo "🎯 Quality Gates:"
...
coverage:
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	@echo "📊 Rust Coverage Analysis (cargo-llvm-cov)"
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	@echo ""
	@mkdir -p target/coverage
	@echo "🦀 Running Rust coverage (excluding WASM-only modules)..."
	@which cargo-llvm-cov > /dev/null 2>&1 || (echo "📦 Installing cargo-llvm-cov..." && cargo install cargo-llvm-cov)
	@cargo llvm-cov --workspace --all-features --ignore-filename-regex='(wasm_init|dom)\.rs' --html --output-dir target/coverage
	@echo ""
	@echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
	...
	@echo ""
	@echo "💡 Note: WASM-only modules (dom.rs, wasm_init.rs) excluded"
	@echo "   These require browser testing via probador"
	@echo ""

coverage-check:
	@echo "📊 Checking coverage thresholds (≥95%)..."
	@cargo llvm-cov --package wos-kernel --package wos-shared --package wos-userspace --all-features --fail-under-lines 95
	@echo "✓ Coverage thresholds met (95%+ on core crates)"

coverage-summary:
	@echo "📊 Coverage Summary (core crates)..."
	@cargo llvm-cov --package wos-kernel --package wos-shared --package wos-userspace --all-features 2>&1 | tail -3
...
probar-commands:
	@echo "⌨️  Running command tests..."
	@cargo test --package wos --test probar_commands -- --test-threads=1
...
# ============================================================================
# Property-Based Testing & Jidoka Guards
# ============================================================================

test-proptest:
	@echo "🎲 Running property-based tests (10K inputs each)..."
	@cargo test --workspace proptest -- --test-threads=1
	@echo "✓ Property tests passed"

test-jidoka:
	@echo "🛡️  Running Jidoka invariant tests..."
	@cargo test --package wos-kernel jidoka -- --test-threads=1
	@cargo test --package wos-kernel state::tests::jidoka_tests -- --test-threads=1
	@echo "✓ Jidoka tests passed"

test-apr:
	@echo "🔄 Running APR runtime tests..."
	@cargo test --workspace apr -- --test-threads=1
	@echo "✓ APR tests passed"

test-vmm:
	@echo "💻 Running VMM layer tests..."
	@cargo test --package wos-kernel vmm -- --test-threads=1
	@echo "✓ VMM tests passed"

# Coverage using llvm-cov (faster than tarpaulin)
coverage-fast:
	@echo "📊 Fast coverage with llvm-cov..."
...
coverage-proptest:
	...
	@cargo llvm-cov --workspace --lcov --output-path target/coverage/proptest-lcov.info -- proptest
	@echo "✓ Property test coverage generated"
